...
Screenshots
The screenshots/ folder contains one image per query result, matching the section headings in elearning_analysis.sql: database & table creation, sample data, INNER/LEFT/RIGHT joins, and Q1 through Q12.
Notes
...
●The database is dropped and recreated at the start of the script (DROP DATABASE IF EXISTS), so re-running it will not cause duplicate-key errors.
